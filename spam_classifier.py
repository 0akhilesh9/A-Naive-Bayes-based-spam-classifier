"""
Naive Bayes based email spam classifier
"""
...
def train_model(train_data, feature_flag = False):
  ...
  # iterate over the training data
    for train_sample in train_data:
      ...
      subject = ""
      from_ = ""
      filepath = core_data_path + train_sample[0]    # filepath for the actual email file
      email = ""
...
def classify_more_features(test_sample, alpha):
  ...
  for mail_type in doc_count_dict.keys():
    ...
    subject = ""
    from_ = ""
    filepath = core_data_path + test_sample[0]
    email = ""
    with open(filepath, 'r') as file_obj:
...
def update(alpha, incorrectHam, incorrectSpam, correctHam, correctSpam):
  ...
  metrics_df = metrics_df.append(pd.DataFrame([[alpha, incorrectHam, correctHam, incorrectSpam, correctSpam, precision, recall, fmeasure]],columns=columns))
  
      
if __name__ == "__main__":
  usageStr = """
      USAGE:      python spam_classifier.py --f1 <train_dataset> --f2 <test_dataset> --o <output_file>
      EXAMPLE:    python spam_classifier.py --f1 train --f2 test --o output.csv
      """
